...
import com.mcennis.archivedj.AudioArrays;
...
public class AudioSamples {
	 ...
	 */
	public AudioSamples(File audio_file, String unique_identifier,
		...
		if (audio_file.getName().endsWith(".ogg")) {
			channel_samples = AudioArrays.extract_sample_values(audio_file);
			audio_format = new AudioFormat(44100, 16, 2, true, true);
		} else {
			AudioInputStream audio_input_stream = null;

			try {
				audio_input_stream = AudioSystem
	 ...
	 * @throws Exception
	 *             Throws an informative exception if the samples cannot be
	 *             extracted from the AudioInputStream.
	 */
	public AudioSamples(AudioInputStream audio_input_stream,
			String unique_identifier, boolean normalize_if_clipped)
			throws Exception {
	 ...
	 * @throws Exception
	 *             Throws an informative exception if the samples cannot be
	 *             extracted from the audio_samples.
	 */
	public AudioSamples(double[][] audio_samples, AudioFormat audio_format,
			String unique_identifier, boolean normalize_if_clipped)
			throws Exception {
		...
		for (int chan = 0; chan < audio_samples.length; chan++)
			if (audio_samples[chan] == null)
				throw new Exception(
						"One or more channels of given audio samples array is empty.");
		int number_samples = audio_samples[0].length;
		for (int chan = 0; chan < audio_samples.length; chan++)
			if (audio_samples[chan].length != number_samples)
				throw new Exception(
						"Different channels of given audio samples array have a\n"
								+ "different number of samples.");
		if (audio_format == null)
			throw new Exception("Null audio format specified for samples.");
		if (audio_format.getChannels() != audio_samples.length)
			throw new Exception("The specified samples have "
					+ audio_samples.length + " channels but\n"
					+ "the specified audio format has "
	 ...
	 *             extracted from the audio_samples.
	 */
	public AudioSamples(double[][] audio_samples, float sampling_rate,
			String unique_identifier, boolean normalize_if_clipped)
			throws Exception {
		if (audio_samples == null)
			throw new Exception("Given audio samples array is empty.");
	 ...
	 * references to the original fields, so no changes made to the copies will
	 * change the original
	 * 
	...
	public AudioSamples getCopyOfAudioSamples() throws Exception {
		...
		if (audio_format != null) {
			new_audio_format = new AudioFormat(audio_format.getEncoding(),
					...
					audio_format.getChannels(), audio_format.getFrameSize(),
					audio_format.getFrameRate(), audio_format.isBigEndian());
		}
	...
	/**
	 * Returns a formatted description of the AudioFormat of the samples as well
	 * as the number of samples per channel, the duration in seconds of the
	 * recording and the maximum sample amplitude.
	 * 
	 ...
	 * 
	 * @return The sampling rate associated with the stored samples in the form
	 *         of a float.
	 */
	 ...
	 * of multiple channels, then the returned samples represent the audio after
	...
	/**
	 * Returns the stored audio samples between the given sample indices. If the
	 * audio data originally consisted of multiple channels, then the returned
	 * samples represent the audio after mixing down into a single channel.
	 * 
	 * @param start_sample
	 *            The sample indice of the first sample to return. Must be 0 or
	 *            higher and must be less than end_sample.
	 * @param end_sample
	 *            The sample indice of the last sample to return. Must be less
	 *            than the total number of samples.
	 * @return The audio samples stored in this object. These have a minimum
	 *         value of -1 and a maximum value of +1. Only the samples within
	 *         (and including) the given sample indices are returned. Only one
	 *         (potentially combined) channel of audio is returned.
	 * @throws Exception
	 *             Throws an informative exception if the given start sample or
	 *             end sample are outside acceptable ranges.
	 */
	public double[] getSamplesMixedDown(int start_sample, int end_sample)
			throws Exception {
		if (start_sample < 0)
			throw new Exception("Requested audio starting at sample "
					+ start_sample
					+ "\nStart sample indice must be 0 or greater.");
		if (end_sample >= samples.length)
			throw new Exception(
					"Requested audio ending at sample "
							+ end_sample
							+ "\nA total of "
							+ samples.length
							+ "samples are present."
		...
		if (start_sample >= end_sample)
			throw new Exception(
					"Requested audio starting at sample "
							+ start_sample
							+ " and ending at sample "
							+ end_sample
							+ ".\n"
	 ...
	 * data originally consisted of multiple channels, then the returned samples
	 * represent the audio after mixing down into a single channel.
	 * 
	 * @param start_time
	 *            The time, in seconds, of the first sample to return. Must be 0
	 *            or higher and must be less than end_time.
	 * @param end_time
	 *            The time, in seconds, of the last sample to return. return.
	 *            Must be less than or equal to the total duration.
	 * @return The audio samples stored in this object. These have a minimum
	 *         value of -1 and a maximum value of +1. Only the samples within
	 *         (and including) the given times are returned. Only one
	 *         (potentially combined) channel of audio is returned.
	 * @throws Exception
	 *             Throws an informative exception if the given times are
	 ...
	 * 
	 * @param window_size
	 *            The length in samples of the windows that the samples are to
	 *            divided into.
	 * @return The audio samples stored in this object after being broken into
	 *         equally sized windows of the specified size. The last window is
	 *         zero-padded if necessary. The samples have a minimum value of -1
	 *         and a maximum value of +1. Only one (potentially combined)
	 *         channel of audio is returned. The first indice specifies the
	 *         window and the second indice specifies the sample index within
	 *         the window.
	 * @throws Exception
	 *             Throws an exception if a negative or 0 window size is
	 *             specified.
	 */
	public double[][] getSampleWindowsMixedDown(int window_size)
			throws Exception {
		if (window_size < 1)
			throw new Exception("Window size of " + window_size
		...
		for (int win = 0; win < number_windows; win++) {
			if (win != number_windows - 1) {
				for (int samp = 0; samp < window_size; samp++)
					windowed_samples[win][samp] = samples[win * window_size
							+ samp];
			} else {
				for (int samp = 0; samp < window_size; samp++) {
	 ...
	 *            The duration in seconds of the windows that the samples are
	 *            divided into.
	 * @return The audio samples stored in this object after being broken into
	 *         equally sized windows of the specified size. The last window is
	 *         zero-padded if necessary. The samples have a minimum value of -1
	...
	/**
	 * Returns the stored audio samples in the form of an AudioInputStream. If
	 ...
	 * channel.
	 * 
	 * @return The entire set of samples mixed down into one channel. Returned
	 *         in the form of an AudioInputStream.
	 * @throws Exception
	 *             Throws an exception if a a problem occurs during conversion.
	 */
	public AudioInputStream getAudioInputStreamMixedDown() throws Exception {
		// Specify that only one channel is used
		AudioFormat mixed_down_audio_format = new AudioFormat(
				audio_format.getSampleRate(),
				audio_format.getSampleSizeInBits(), 1, true,
	 ...
	 * channels, then the returned samples represent the audio after mixing down
	 ...
	 * @throws Exception
	 *             Throws an informative exception if the given start sample or
	 *             end sample are outside acceptable ranges or if error occurs
	 *             during conversion.
	 */
	public AudioInputStream getAudioInputStreamMixedDown(int start_sample,
			int end_sample) throws Exception {
		// Specify that only one channel is used
		AudioFormat mixed_down_audio_format = new AudioFormat(
				audio_format.getSampleRate(),
				audio_format.getSampleSizeInBits(), 1, true,
	 ...
	 *         Returned in the form of an AudioInputStream.
	 * @throws Exception
	 *             Throws an informative exception if the given start time or
	 *             end time are outside acceptable ranges or if error occurs
	 *             during conversion.
	 */
	public AudioInputStream getAudioInputStreamMixedDown(double start_time,
			double end_time) throws Exception {
		// Specify that only one channel is used
		AudioFormat mixed_down_audio_format = new AudioFormat(
				audio_format.getSampleRate(),
				audio_format.getSampleSizeInBits(), 1, true,
	 ...
	 *         corresponds to the channel and the second indice corresponds to
	 *         the sample number.
	 */
	 ...
	 *             end sample are outside acceptable ranges.
	 */
	public double[][] getSamplesChannelSegregated(int start_sample,
			int end_sample) throws Exception {
		if (start_sample < 0)
			throw new Exception("Requested audio starting at sample "
					+ start_sample
	 ...
	 *         value of -1 and a maximum value of +1. The first indice
	 *         corresponds to the channel and the second indice corresponds to
	 *         the sample number. Only the samples within (and including) the
	 *         given times are returned.
	 * @throws Exception
	 *             Throws an informative exception if the given times are
	 *             outside acceptable ranges.
	 */
	public double[][] getSamplesChannelSegregated(double start_time,
			double end_time) throws Exception {
		int start_sample = convertTimeToSampleIndex(start_time);
		int end_sample = convertTimeToSampleIndex(end_time);
	 ...
	 *         equally sized windows of the specified size. The last window is
	 *         zero-padded if necessary. The samples have a minimum value of -1
	 *         and a maximum value of +1. The first indice specifies the
	 *         channel, the second indice specifies the window and the third
	 *         indice specifies the sample index within the window.
	 * @throws Exception
	 *             Throws an exception if a negative or 0 window size is
	 *             specified.
	 */
	public double[][][] getSampleWindowsChannelSegregated(int window_size)
		...
		for (int chan = 0; chan < channel_samples.length; chan++)
			for (int win = 0; win < number_windows; win++) {
				if (win != number_windows - 1) {
					for (int samp = 0; samp < window_size; samp++)
						windowed_samples[chan][win][samp] = channel_samples[chan][win
								* window_size + samp];
				} else {
					for (int samp = 0; samp < window_size; samp++) {
						if (win * window_size + samp < samples.length)
	 ...
	 * 
	 * @return The entire set of samples returned in the form of an
	 *         AudioInputStream.
	 * @throws Exception
	 *             Throws an exception if a a problem occurs during conversion.
	 */
	public AudioInputStream getAudioInputStreamChannelSegregated()
		...
		// Convert to an AudioInputStream
		AudioInputStream audio_input_stream = AudioMethods
				.convertToAudioInputStream(samples_to_convert, audio_format);
	 ...
	 *            The sample indice of the last sample to return. Must be less
	 *            than the total number of samples.
	 * @return The specified set of samples returned in the form of an of an
	 *         AudioInputStream.
	 * @throws Exception
	 *             Throws an informative exception if the given start sample or
	 *             end sample are outside acceptable ranges or if error occurs
	 ...
	 */
	public AudioInputStream getAudioInputStreamChannelSegregated(
			int start_sample, int end_sample) throws Exception {
		// Extract samples from samples field if only one channel present
		double[][] samples_to_convert = getSamplesChannelSegregated(
	 ...
	 */
	public AudioInputStream getAudioInputStreamChannelSegregated(
			double start_time, double end_time) throws Exception {
		// Extract samples from samples field if only one channel present
		double[][] samples_to_convert = getSamplesChannelSegregated(start_time,
				end_time);
	 ...
	 * @throws Exception
	 *             Throws an informative exception if the samples cannot be
	 *             succesfully saved to the file.
	 */
	public void saveAudio(File save_file, boolean multi_channel,
				...
				for (int samp = 0; samp < channel_samples[chan].length; samp++)
					if (Math.abs(channel_samples[chan][samp]) > 1.0) {
						double difference = Math
								.abs(channel_samples[chan][samp]) - 1.0;
						if (difference > max_difference)
							max_difference = difference;
					}
		} else
	...
	/**
	 * Normalizes the channel segregated samples so that the absolute value of
	 * the highest sample amplitude is 1. Does nothing if all samples are 0.
	 */
	...
	/**
	 * Normalizes both the channel segregated samples and the samples mixed down
	 * into one channel so that the absolute value of the highest sample
	 ...
	 * samples must have the same number of channels as the original data. The
	 * stored samples are a copy of the given samples, so changes to the passed
	 * array will not affect the information stored here.
	 * 
	 * @param new_samples
	 *            Audio samles to store, usually with a minimum value of -1 and
	 *            a maximum value of +1. The first indice corresponds to the
	 *            channel and the second indice corresponds to the sample
	 *            number.
	 * @throws Exception
	 *             Throws an exception if an invalid parameter is passed (null
	 *             entries, non-matching number of channels or samples per
	 *             channel.
	 */
	public void setSamples(double[][] new_samples) throws Exception {
		...
		for (int chan = 0; chan < new_samples.length; chan++) {
			if (new_samples[chan] == null)
				throw new Exception("Channel " + chan
						+ " of the given samples is empty.");
			if (number_samples != -1)
				if (number_samples != new_samples[chan].length)
					throw new Exception(
							"Different channels have different numbers of samples.");
		...
		if (channel_samples == null) {
			if (new_samples.length != 1) {
				throw new Exception("Given samples have " + new_samples.length
						+ " channels.\n"
			...
			samples = DSPMethods
					.getSamplesMixedDownIntoOneChannel(channel_samples);
	 ...
	 * 
	 * @param sampling_rate
	 *            The sampling rate, in Hz, to use for interpereting samples.
	 * @return The default AudioFormat with the specified sampling rate.
	 */
	 ...
	 * 
	 * @param sample_index
	 *            The sample index to convert to a time value.
	 * @return The time in seconds corresponding to the given sample index, or
	 *         the nearest time if the given sample index is outside the
	...
	/**
	 * Returns the given time in seconds converted into a sample index based on
	 * the sampling rate of the stored audio samples. If the given time would
	 * correspond to a negative sample, then the index of the first sample is
	 * returned. If the given time would correspond to a sample greater than the
	 * length of the audio, then the last index is returned.
	 * 
	 * @param time
	 *            The time in seconds to convert to a sample.
	 * @return The sample corresponding to the given time, or the nearest sample
	 *         if the given time is outside the duration of the audio.
	 */
